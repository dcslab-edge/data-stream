...
longs_value_filtered = longs_value.map(lambda x: 0 if x>10000 else 1).map(lambda x: ('l',x))
ints_value_filtered = ints_value.map(lambda x: 0 if x >= 50 and x<80 else 1).map(lambda x: ('i',x))
